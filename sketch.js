...
function preload() {
  bgnew = loadImage("data/bgnew.png");
  bgnew2 = bgnew;
...
//------------------------------------------------***BACKGROUND***----------------------------------------------
//creates a background marquee
function marquee() {
  imageMode(CORNER);
  image(bgnew, xInit, 0);
  image(bgnew2, xInit2, 0);
  if (secondScroll && !endGame) {
    xInit2 = xInit2 + curS;
  }
  if (firstScroll && !endGame) {
    xInit = xInit + curS;
  }
  if (xInit >= -10 && !secondScroll) {
    secondScroll = true;
  }
  if (xInit2 >= -10 && !firstScroll) {
    firstScroll = true;
  }
  if (xInit >= 590 && firstScroll) {
    firstScroll = false;
    secondScroll = true;
    xInit = -3200;
  }
  if (xInit2 >= 590 && secondScroll) {
    firstScroll = true;
    secondScroll = false;
    xInit2 = -3200;
  }
...
function Player() {
  ...
  this.collision = function(other) {
    ...
    var col1 = ((this.x <= otherLocUpperX) && (this.x >= otherLocLowerX) && (this.y >= otherLocLowerY) && (this.y <= otherLocUpperY));
    ...
    var col2 = ((other.x <= thisLocUpperX) && (other.x >= thisLocLowerX) && (other.y >= thisLocLowerY) && (other.y <= thisLocUpperY));
    return col1 || col2;
  }
      ...
      this.f.forEach(function(obj) {
        ...
        bullets.b.forEach(function(bull) {
          if (bull.collision(obj)
            ...
            && (bull.alreadyTaken == null || bull.alreadyTaken == obj) && (obj.thisHit == null || obj.thisHit == bull)) {
